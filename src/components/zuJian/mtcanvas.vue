  ...
  export default{
    mounted() {
      ...
      var Main = function() {
        ...
        var dotsDistance = 250; // 点之间产生连线的最大距离
        ...
        function animateUpdate() {
          ...
          // 绘制连线
          for (var t = overNum; t < dotsNum; t++) {
              for (var j = t + 1; j < dotsNum; j++) {
                var tx = dotsArr[t].x - dotsArr[j].x
                var ty = dotsArr[t].y - dotsArr[j].y
                var s = Math.sqrt(Math.pow(tx, 2) + Math.pow(ty, 2));
                if (s < dotsDistance) {
                  ctx.beginPath();
                  ctx.moveTo(dotsArr[t].x, dotsArr[t].y);
...
<style>
  ...
  #bg{
    background: url(../../assets/starsky.jpg) no-repeat;
